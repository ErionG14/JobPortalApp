...
const EditJobScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (jobData) {
      try {
        const parsedJob: ManagerPostedJob = JSON.parse(jobData as string);
        ...
        setInitialLoadError("Failed to load job details. Please try again.");
      }
    } else {
      setInitialLoadError("No job data provided for editing.");
    }
  }, [jobData]);
  ...
  const handleSubmit = async () => {
    ...
    if (!user || !user.token || user.role !== "Manager" || jobId === null) {
      Alert.alert(
        "Access Denied",
        "Only managers can edit jobs or job ID is missing. Please log in as a manager."
      );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          "API Response not OK for UpdateJob:",
          response.status,
          errorText
        );
    ...
    return (
      <SafeAreaView className="flex-1 justify-center items-center bg-gray-50 p-4">
        <Text className="text-red-600 text-lg text-center">
          Access Denied: Only Managers can edit jobs.
        </Text>
        <TouchableOpacity
